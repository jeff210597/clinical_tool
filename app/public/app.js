...
function renderAuth() {
  ...
  if (loggedIn) {
    ...
    el.loginMessage.textContent = "請使用 Onepage 帳號登入；密碼僅用於本次登入，不會儲存在工作台。";
...
function renderPatient(patient, options = {}) {
  ...
  const sourceLabel =
    ...
    patient.source === "missing_auth" ? "Onepage 登入已失效" :
    "Onepage/NIS";
...
function roundingSummary(patient) {
  ...
  return `
    ...
    ${patient.source === "missing_auth" ? `<div class="session-help"><strong>Onepage 登入已失效</strong><p>請登出後以 Onepage 帳號與密碼重新登入工作台。</p></div>` : ""}
  `;
}
...
function historyLine(patient) {
  ...
  if (rows.length) return rows.map((row) => row.label || row.text).filter(Boolean).join("；");
...
function renderLabs(patient) {
  const history = patient.labHistory || buildClientLabHistory(patient.labs || [], 0);
  if (patient.labView === "all") return `${renderLabHistoryControls(history)}${renderAllLabs(filterLabRowsByHistory(patient.labs || [], history))}`;
  const matrix = history.labMatrix || patient.labMatrix;
  const controls = renderLabHistoryControls(history);
...
function buildClientLabHistory(rows = [], offset = 0, range = {}) {
  const availableDates = [...new Set(rows.map((row) => row.time).filter(Boolean))];
  const startIndex = availableDates.indexOf(range.startDate);
  const endIndex = availableDates.indexOf(range.endDate);
  const from = startIndex === -1 || endIndex === -1 ? 0 : Math.min(startIndex, endIndex);
  const to = startIndex === -1 || endIndex === -1 ? availableDates.length - 1 : Math.max(startIndex, endIndex);
  const dates = availableDates.slice(from, to + 1);
  const safeOffset = Math.max(0, Math.min(Number(offset) || 0, Math.max(0, dates.length - 1)));
  const columns = dates.slice(safeOffset, safeOffset + 3);
  ...
  return {
    ...
    labMatrix: buildClientLabMatrix(selected, columns),
  };
}

function buildClientLabMatrix(rows = [], columns = []) {
  const items = [...new Set(rows.map((row) => row.item).filter(Boolean))];
  return {
    columns,
    rows: items.map((item) => {
      const sample = rows.find((row) => row.item === item) || {};
      const values = Object.fromEntries(columns.map((column) => {
        const found = rows.find((row) => row.item === item && row.time === column);
        return [column, found ? { value: found.latest || "", unit: found.unit || "", flag: normalizeLabFlag(found), rawFlag: found.flag || "", ref: found.ref || "" } : null];
      }));
      return { item, group: sample.group || inferLabGroup(item), unit: sample.unit || "", ref: sample.ref || "", values };
    }),
  };
}
...
function filterLabRowsByHistory(rows, history) {
  const dates = history?.availableDates || [];
  const start = dates.indexOf(history?.startDate);
  const end = dates.indexOf(history?.endDate);
  if (start === -1 || end === -1) return rows;
  const selected = new Set(dates.slice(Math.min(start, end), Math.max(start, end) + 1));
  return rows.filter((row) => selected.has(row.time));
}
...
function notePanel(patient) {
  ...
  const picker = progress.length ? `<label>Progress 日期 <select id="noteProgressPicker">${progress.map((row) => `<option value="${escapeHtml(row.id || "")}">${escapeHtml(row.date || "未提供日期")}</option>`).join("")}</select></label>` : `<span>目前沒有可選的 Progress 索引。</span>`;
  return `<section class="record-block"><h3>OneRecord Note</h3><p>點選後才讀取原始文件；若原始系統未回應，會顯示「病歷尚未完成」。</p><div class="context-actions"><button class="note-open" data-note-type="admission" type="button">入院病摘</button><button class="note-open" data-note-type="progress" type="button" ${progress.length ? "" : "disabled"}>Progress</button><button class="note-open" data-note-type="discharge" type="button">出院病摘</button>${picker}</div><div id="noteDocument" class="empty-state">請選擇要檢視的病歷。</div></section>`;
}
...
function surgicalDiagnosisEvidence(items = []) {
  if (!items.length) return `<div class="empty-state compact">尚未擷取可驗證的手術術後診斷。</div>`;
  const effectLabel = {
    current_support: "同次住院：交叉支持現有診斷",
    past_history: "既往住院：列入過去病史",
    review_only: "未驗證：僅供手術分頁人工核對",
  };
  return records(items.map((item) => ({
    title: [item.date, item.diagnosis].filter(Boolean).join(" · ") || "手術術後診斷",
    body: [
      item.procedure && `手術：${item.procedure}`,
      effectLabel[item.effect] || "未驗證",
      item.admissionEvidence && `判定依據：${item.admissionEvidence}`,
    ].filter(Boolean).join("；"),
  })));
}
...
function missingDataState(message = "尚未擷取資料。請輸入病歷號或床號查詢；若仍無資料，可能是 Onepage 登入已失效或此來源 parser 尚未接上。") {
...
async function loadPatient(query, options = {}) {
  ...
  if (!silent) {
    ...
    void loadPatientDetails(query, patientKey(patient));
  }
  if (updateRecent) loadRecent().catch(() => null);
  return patient;
}

async function loadPatientDetails(query, expectedKey, sources = ["orders", "admission", "progress", "discharge", "adult_assessment", "imaging", "surgeries", "pathology", "nursing"]) {
  const loadKey = `${expectedKey}:${[...sources].sort().join(",")}`;
  if (state.detailLoadingKey === loadKey) return;
  state.detailLoadingKey = loadKey;
  try {
    const patient = await api("/api/patients/search", { method: "POST", body: JSON.stringify({ query, mode: "details", sources }) });
    if (patientKey(state.currentPatient) !== expectedKey) return;
    mergePatientLoad(patient, state.currentPatient);
    renderPatient(patient);
    switchTab(state.currentTab);
    el.patientMeta.textContent = patient.cacheStatus === "memory_cache" ? "詳細資料已由快取載入" : "詳細資料已更新";
  } catch {
...
function switchTab(name) {
  ...
  if (sources.length && state.currentPatient && !sources.every((source) => state.currentPatient.loadedSources?.includes(source))) {
    ...
    void loadPatientDetails(query, patientKey(state.currentPatient), sources);
  }
...
document.addEventListener("click", async (event) => {
  ...
  if (event.target?.classList?.contains("lab-range-apply") && state.currentPatient) {
    ...
    state.currentPatient.labHistory = buildClientLabHistory(state.currentPatient.labs || [], 0, { startDate, endDate });
    renderPatient(state.currentPatient, { store: false });
    switchTab("labs");
    return;
  }
  if (event.target?.classList?.contains("lab-history-all") && state.currentPatient) {
    state.currentPatient.labView = "all";
    renderPatient(state.currentPatient, { store: false });
    switchTab("labs");
    return;
  }
  const labHistoryOffset = event.target?.dataset?.labHistoryOffset;
  if (labHistoryOffset !== undefined && state.currentPatient && !state.labHistoryLoading) {
    const previous = state.currentPatient.labHistory || {};
    const history = buildClientLabHistory(state.currentPatient.labs || [], Number(labHistoryOffset), previous);
    state.currentPatient.labHistory = history;
    state.currentPatient.labView = "matrix";
    renderPatient(state.currentPatient, { store: false });
    switchTab("labs");
    return;
  ...
  if (event.target?.classList?.contains("note-open")) {
    ...
    const noteId = noteType === "progress" ? document.querySelector("#noteProgressPicker")?.value || "" : "";
    target.disabled = true;
    const result = await api("/api/patients/note", {
      method: "POST",
      body: JSON.stringify({ patientRef: state.currentPatient.patientRef || state.currentPatient.chartNo, noteType, noteId }),
    });
    const panel = document.querySelector("#noteDocument");
    if (panel) {
      if (result.status === "available" && result.embedUrl) {
        panel.innerHTML = `<div class="record-item"><strong>${escapeHtml([result.note?.title, result.note?.date, result.note?.author].filter(Boolean).join(" · "))}</strong><iframe title="OneRecord Note" src="${escapeHtml(result.embedUrl)}" style="width:100%; min-height:70vh; border:0; margin-top:10px"></iframe></div>`;
      } else {
        panel.textContent = result.status === "unfinished" ? "病歷尚未完成" : (result.message || "OneRecord 文件暫時無法開啟。");
      }
    }
...
el.loginForm.addEventListener("submit", async (event) => {
  ...
  el.loginMessage.textContent = "正在登入 Onepage…";
  try {
    ...
    el.loginMessage.textContent = error.message || "Onepage 登入失敗，請確認帳號與密碼。";
  }
});
...
el.reloadRecent.addEventListener("click", async () => {
  ...
  try {
    ...
    updateRecentStatus("清單讀取失敗，請確認登入狀態。");
  } finally {
